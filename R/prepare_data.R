...
if (file.exists(hurdat_file) & !recook) {
  ...
  landfall_time <- parse_date_time(landfalls$hours, "HM")
...
## below this is old code, we've since updated the data sources.
# I'll delete it, but this comment and the code will at least live in an
# old commit for posterity. 
...
#  rainfall_table <- rvest::html_table(rainfall_max, header=TRUE)[[1]]
#
#  nrows <- nrow(rainfall_table)
#
#  parsed_table <- lapply(2:nrows, function(i) { 
#
#      value_list <- html_nodes(rainfall_max,"tr")[[i]] %>%
#          html_nodes("td") %>% 
...
#          str_split("\n")
#
#      values <- lapply(value_list, function(x) {
#             x <- str_replace_all(x, "\\s+", "")
#             x[x!= ""] 
#      })
#
#      name <- paste0(values[[1]], collapse="")
#      amount <- as.numeric(values[[2]])
#      over_idx <- 1:(1+length(values[[3]])-length(amount))
#      locs1 <- str_c(values[[3]][over_idx], collapse="")
#      locs2 <- values[[3]][-over_idx]
#      locs <- c(locs1, locs2)
#     
#      print(i) 
#      res <- data.frame(name = name, location = locs, amount=as.numeric(values[[2]])) 
#      return(res)
#  })
#
#  noutrows <- sum(lapply(res, length)) 
#
#  i <- 1
#  out_df <- data.frame(matrix(ncol=3,nrow=noutrows))
#  names(out_df) <- c("name", "month", "year", "location", "amount")
#  lapply(parsed_table, function(table_values) {
#      
#  })
#
#  #   
...
#storms_with_sccr_surge <- lapply(unique(storm_surge_stations$station)[1:10], function(surge_station) {
...
#          dir.create(sprintf("./data/gssr/raw/%s/",station_name),recursive=TRUE) 
#  }
...
#                 max_surge_recon <- max(station_recon_filtered$surge_reconstucted,na.rm=TRUE) 
